function Sockets(io, screenless, config) {
    if (!(this instanceof Sockets)) return new Sockets(io, screenless, config);
    this.io = io;
    this.config = config;
...
Sockets.prototype.registerScreen = function(socket, data) {
    ...
    if (this.registerClient(socket, 'screen', data)) {
        ...
        socket.on('screen-to-server', function(key, payload) {
            if (floodControl(socket.una.id, key)) {
                if (screenless && (socket.una.room in states)) {
...
Sockets.prototype.registerController = function(socket, data) {
    ...
    if (this.registerClient(socket, 'controller', data)) {
        ...
        socket.on('controller-to-server', function(key, payload) {
            if (floodControl(socket.una.id, key)) {
                if (screenless && (socket.una.room in states)) {
                    states[socket.una.room].onControllerInput(socket.una, key, payload);
                }
            }
        })
